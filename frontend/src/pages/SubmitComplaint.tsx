...
const SubmitComplaint = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Use configured axios instance with correct prefix
      const response = await axios.post("/api/public/complaint", formData);
      alert(
        `Complaint Submitted! Your Reference ID is: ${response.data.reference_id}`
      );
      navigate("/");
    } catch (error) {
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 p-8 border border-slate-100">
          <div className="mb-8">
            ...
            <p className="text-slate-500 mt-2">
              Please provide details about the incident. Your privacy is
              important to us.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  <span className="flex items-center">
                    <Upload className="w-4 h-4 mr-1" /> Photo Evidence
                    (Optional)
                  </span>
                </label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  <span className="flex items-center">
                    <ShieldCheck className="w-4 h-4 mr-1" /> National ID (NIN)
                    (Optional)
                  </span>
                </label>
                <input
